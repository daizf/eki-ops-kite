...
func syncAllPools(ctx context.Context, interval time.Duration) {
	...
	if err != nil {
		klog.Errorf("KCS sync: failed to list pools: %v", err)
		return
	}

	for _, pool := range pools {
		if !pool.Enable || pool.KcsBaseURL == "" {
			continue
		}

		s := newSyncer(pool, interval)
		klog.Infof("KCS sync: syncing pool %s (%s), baseURL=%s, proxy=%s", pool.PoolID, pool.PoolName, s.baseURL, pool.Proxy)
		if err := s.sync(ctx); err != nil {
			klog.Errorf("KCS sync: pool %s sync failed: %v", pool.PoolID, err)
		}
	}
}